...
{  
    ...
    double A = ms.at<double>(0,0);  
    double B = ms.at<double>(1,0);  
...
//calc the n-th params' partial derivation M-oM-<M-^L the params are our  final target  
...
{  
    ...
    Mat params1 = ms.clone();  
    Mat params2 = ms.clone();  
...
{  
    ...
    for(i=0; i < MAX_ITER; i++) {  
        ...
        for(int j=0; j < m; j++) 
        {
            ...
            // 计算残差
            r.at<double>(j,0) = outputs.at<double>(j,0) - Func(input, ms);//diff between previous estimate and observation population  
            ...
            for(int k=0; k < num_params; k++) {  
                Jf.at<double>(j,k) = Deriv(Func, input, ms, k);  //construct jacobian matrix  
            }  
        }  
